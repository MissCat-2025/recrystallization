// ADBulkFreeEnergyDensity2.C
#include "ADBulkFreeEnergyDensity2.h"

registerMooseObject("RecrystallizationApp", ADBulkFreeEnergyDensity2);

InputParameters
ADBulkFreeEnergyDensity2::validParams()
{
  ...
  params.addClassDescription("计算多晶与气泡相的体自由能密度材料，并利用自动微分计算对变量的导数");
  
  return params;
}

ADBulkFreeEnergyDensity2::ADBulkFreeEnergyDensity2(const InputParameters & parameters)
  : ADMaterial(parameters),
    _c_g(adCoupledValue("c_g")),
    ...
    _f_bulk(declareADProperty<Real>(getParam<MaterialPropertyName>("f_name")))
{
  ...
  for (unsigned int i = 0; i < _num_grains; ++i)
    _grain_vars[i] = &adCoupledValue("grain_etas", i);
...
void
ADBulkFreeEnergyDensity2::computeQpProperties()
{
  // 计算多晶结构相序参数平方和
  ADReal sum_grain_eta_sq = 0.0;
  for (unsigned int i = 0; i < _num_grains; ++i)
    sum_grain_eta_sq += (*_grain_vars[i])[_qp] * (*_grain_vars[i])[_qp];
  
  // 计算所有序参数平方和
  ADReal sum_all_eta_sq = sum_grain_eta_sq;
  for (unsigned int i = 0; i < _num_bubbles; ++i)
    sum_all_eta_sq += (*_bubble_vars[i])[_qp] * (*_bubble_vars[i])[_qp];
  
  // 计算θ_m和θ_b
  ADReal theta_m = sum_grain_eta_sq / sum_all_eta_sq;
  ADReal theta_b = 1.0 - theta_m;
  
  // 计算f_g^m和f_g^b
  ADReal c_g_diff_m = _c_g[_qp] - _c_m_e;
  ADReal c_g_diff_b = _c_g[_qp] - _c_b_e;
  ADReal f_g_m = _S_I * c_g_diff_m * c_g_diff_m;
  ADReal f_g_b = _S_I * c_g_diff_b * c_g_diff_b;
  
  // 计算各项总和
  ADReal sum_eta2_eta4 = 0.0;
  for (unsigned int i = 0; i < _num_grains; ++i)
  ...
  // 计算晶粒间相互作用
  ADReal sum_grain_interaction = 0.0;
  for (unsigned int i = 0; i < _num_grains; ++i)
  ...
  // 计算气泡与晶粒间相互作用
  ADReal sum_bubble_grain_interaction = 0.0;
  for (unsigned int i = 0; i < _num_bubbles; ++i)
    for (unsigned int j = 0; j < _num_grains; ++j)
